...
class LinearModel(ModelBuilder):
    ...
    def build_model(self, X: pd.DataFrame, y: pd.Series, **kwargs):
        ...
        with pm.Model(coords=self.model_coords) as self.model:
            ...
            x_data1 = pm.Data("x_data1", X_values["total_pace"], dims="obs_id", mutable=True)
            x_data2 = pm.Data("x_data2", X_values["curr_pace"], dims="obs_id", mutable=True)
            ...
            b2_mu_prior = self.model_config.get("b2_mu_prior", 0)
            b2_sigma_prior = self.model_config.get("b2_sigma_prior", 5)
            ...
            b_2 = pm.Normal("b_2", mu=b2_mu_prior, sigma=b2_sigma_prior, dims="group")
            ...
            obs = pm.Normal("y", mu=b_0[g] + b_1[g] * x_data1 + b_2[g] * x_data2, sigma=sigma[g], shape=x_data1.shape, observed=y_data, dims="obs_id")
    ...
    def _data_setter(
        ...
        if isinstance(X, pd.DataFrame):
            ...
            x_values = X[["total_pace", "curr_pace"]] #["total_pace"]#X   #["input"].values
        else:
        ...
        with self.model:
            pm.set_data({"x_data1": x_values["total_pace"], "x_data2": x_values["curr_pace"], "g": dist_idx}, coords=self.model_coords)
            if y is not None:
